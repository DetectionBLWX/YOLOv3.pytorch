...
class COCODataset(Dataset):
	def __init__(self, rootdir, image_size_dict, max_num_gt_boxes, use_color_jitter, img_norm_info, mode, **kwargs):
	...
	def __getitem__(self, index):
		if self.mode == 'TRAIN':
			...
			img, scale_factor, target_size = COCODataset.preprocessImage(img, use_color_jitter=self.use_color_jitter, image_size_dict=self.image_size_dict, img_norm_info=self.img_norm_info)
	...
	@staticmethod
	def preprocessImage(img, use_color_jitter, image_size_dict, img_norm_info):
		...
		means_norm = img_norm_info.get('mean_rgb')
		stds_norm = img_norm_info.get('std_rgb')
		if use_color_jitter:
